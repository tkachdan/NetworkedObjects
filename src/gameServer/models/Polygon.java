...
public class Polygon {
    public PointModel[] shape; // An array of points.
    public PointModel offset; // The offset mentioned above
     ...
     *
     * @param inShape array of Points that defines this polygon's shape
     */
    public Polygon(PointModel[] inShape) {
        this(inShape, new PointModel(0, 0), 0.0);
    }

    public Polygon(PointModel[] flyingShape, PointModel point, double inRotation) {
        this.shape = flyingShape;
        this.offset = point;
        this.rotation = inRotation;

        PointModel origin = (PointModel) shape[0].clone();

        for (PointModel p : shape) {
            if (p.x < origin.x) {
                origin.x = p.x;
            }
            if (p.y < origin.y) {
    ...
    public Polygon(PointModel[] flyingShape, java.awt.Point point, double inRotation) {
     ...
     * Rotates the polygon.
     *
     * @param degrees Number of degrees by which to rotate this polygon clockwise.
     */
     ...
     *
     * @param x value by which to increment this polygon's x-offset
     * @param y value by which to increment this polygon's y-offset
     */
     ...
     *
     * @param brush The object that will do the drawing.
    ...
    public void paint(Graphics brush) {
        ...
        // space
        PointModel[] truePoints = getPoints();
    ...
    public PointModel[] getPoints() {
        PointModel center = findCenter();
        PointModel[] points = new PointModel[shape.length];
     ...
     * algorithm</a>.
     *
     * @param point the possibly-interior point to test
     * @return true if <code>point</code is contained in this polygon
     */

    public boolean contains(PointModel point) {
        PointModel[] points = getPoints();
    ...
    private PointModel findCenter() {
        PointModel sum = new PointModel(0, 0);
        for (int i = 0, j = 1; i < shape.length; i++, j = (j + 1)
                % shape.length) {
        ...
        return new PointModel(Math.abs(sum.x / (6 * area)), Math.abs(sum.y
                / (6 * area)));
    }
